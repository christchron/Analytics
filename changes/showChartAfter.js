...
class ShowChart extends Component {
	...
	handleStartDateChange(startD){
	    ...
	    if (this.props.location.pathname.indexOf('hourly') !== -1){
			console.log("hourly");
			this.props.fetchChartAction.fetchChartData(`hourly?start=${this.state.start}&end=${this.state.end}`)
		} else if (this.props.location.pathname.indexOf('daily') !== -1){
			console.log("daily");
			this.props.fetchChartAction.fetchChartData(`daily?start=${this.state.start}&end=${this.state.end}`)
		} else if (this.props.location.pathname.indexOf('weekly')!== -1){
			console.log("weekly");
			this.props.fetchChartAction.fetchChartData(`weekly?start=${this.state.start}&end=${this.state.end}`)
		} else if (this.props.location.pathname.indexOf('monthly')!== -1){
			console.log("monthly");
			this.props.fetchChartAction.fetchChartData(`monthly?start=${this.state.start}&end=${this.state.end}`)
		}
	}
		...
		if(this.props.posts){
			if (!this.props.posts.chatCounts){
				...
				if (this.state.url.indexOf('hourly') !== -1){
					...
					return (
						<ShowTable data={CCTable} dayHeader={"Year"} mySortFunc = {this.mySortFunc}/>
					);
				}
			}
		}else{
					...
					<div className="col-md-12">
						...
						<div className="col-md-1">
							...
							<input 
					  			type="checkbox" 
					  			checked={ this.state.checked } 
					  			onChange={ this.handleCheckBoxChange } />  Show Chart</label>
